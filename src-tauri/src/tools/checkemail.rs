...
impl Reachables {
    pub async fn checkemails(&self) -> Vec<Reachable> {
        ...
        let proxies = vec![
            ...
            // socks5://identityserver4:4b2606a107223dd0d@socks5.kmoz.dev:8318
        ];
        ...
        let proxy_received: Option<CheckEmailInputProxy> = match _this.proxyurl.clone() {
            Some(m) => {
                // let matcher =  Regex::new(r"(socks5:\/\/)(.+?(?=:)):+(.+?(?=@))@+([0-9a-zA-Z.-]{0,64}?(?=:):)+([0-9]{2,5})").unwrap();
                    ...
                    if !proxy_host_port[0].is_empty() && !proxy_host_port[1].is_empty() {
                        if !proxy_auth[0].is_empty() && !proxy_auth[1].is_empty() {
                            ...
                            Some(CheckEmailInputProxy {
                                ...
                                port: proxy_host_port[1]
                                    .parse::<u16>()
                                    .expect("could not parse port in proxy"),
        ...
        for elem in _this.listemails.clone() {
            ...
            let _ = match proxy_received.clone() {
                Some(m) => {
                    // code
                    input.set_proxy(m);
                    true
            ...
            let _ = match smtptimeout_received.as_millis() >= 300 {
                true => {
            ...
            let child = thread::spawn(move || {
                block_on(async {
                    ...
                    thread_tx
                        .send(Reachable {
                            email: elem.to_string(),
                            status: is_reachable,
                            data: jsondata.unwrap(),
                        })
                        .unwrap();
                })
            });
