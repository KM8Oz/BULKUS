...
impl CheckEmailInputProxy {
    pub async fn check(&self) -> Result<String, String> {
        if self.host.is_empty() || self.port.to_string().is_empty() {
            Err("no proxy".into())
        } else {
            let mut proxy_http =
                reqwest::Proxy::all(format!("socks5://{}:{}", self.host, self.port))
                    .expect("proxy format error!");
            proxy_http = match &self.username {
                Some(user) => match &self.password {
                    Some(pass) => proxy_http.basic_auth(&user, &pass),
                    None => proxy_http,
                },
                None => proxy_http,
            };
            let client = reqwest::Client::builder()
                ...
                .build()
                .expect("client not setuped!");
            let _res = client.get("https://ifconfig.co/ip").send().await;
            if _res.is_ok() {
                let res = _res.unwrap().text().await;
                println!("responce ifconfig.co/ip: {:?}", res);
                if res.is_ok() {
                    Ok(res.unwrap())
                } else {
                    Err("no proxy".into())
