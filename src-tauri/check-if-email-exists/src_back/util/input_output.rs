...
unsafe impl Send for CheckEmailOutput {}
impl Clone for CheckEmailOutput {
    fn clone(&self) -> Self {
        Self { input: self.input.clone(), is_reachable: self.is_reachable.clone(), misc: self.misc.clone(), mx: self.mx.clone()
			, smtp: self.smtp.clone(), syntax: self.syntax.clone(),
		 }
    }
}
	...
	fn should_serialize_correctly() {
		...
		fn dummy_response_with_message(m: &str) -> CheckEmailOutput {
			...
			CheckEmailOutput {
				...
				smtp: Err(super::SmtpError::SmtpError { field1: r.into() }),
			}
		}
